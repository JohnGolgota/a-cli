...
function Setup {
    # responsabilidades de init: agregar las utils y la variable de entorno.
    if (-not(Test-Path $PROFILE)) {
        Write-Host "Creando archivo $PROFILE"
        New-Item -Path $APaths -ErrorAction SilentlyContinue
    }
    $utils = Join-Path $PSScriptRoot "ACli-utils.ps1"
    Add-Content -Path $PROFILE -Value "`n. $utils`n"
    Add-ToPath
}

$A_CLI_BIN = Join-Path $PSScriptRoot "bin"
